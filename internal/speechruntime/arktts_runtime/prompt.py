...
class PromptBuilder:
    ...
    def build(self, target_text: str, reference_text: str, reference_codes: np.ndarray) -> np.ndarray:
        ...
        if int(codes.min()) < 0:
            raise ValueError("reference codes must be non-negative")
        ...
        values[0] = row0
        reference_start = len(prefix)
        values[1:, reference_start : reference_start + codes.shape[1]] = codes
